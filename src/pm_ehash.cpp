...
PmEHash::~PmEHash() {
    if (!disposed){
        ...
        PersistWorker::post_persist(catalog->buckets_pm_address, sizeof(pm_address)<<metadata->global_depth);
...
int PmEHash::remove(uint64_t key) {
    ...
    for (int i = 0; i < BUCKET_SLOT_NUM; i++) {//遍历所有槽位
        if (tar_bucket->bitmap[i] && tar_bucket->slot[i].key == key) {//当前槽位有kv对 且kv对的key与希望删除的key相同
            ...
            //修改后需要持久化
            PersistWorker::post_persist(tar_bucket->bitmap, sizeof(tar_bucket->bitmap));//只需要持久化bitmap而不需要持久化整个桶 保障程序运行性能
            succ=1;//删除成功
            break;
        }
    }   
...
int PmEHash::update(kv kv_pair) {
    ...
    for (int i = 0; i < BUCKET_SLOT_NUM; i++) {//遍历所有槽位
        if (tar_bucket->bitmap[i] && tar_bucket->slot[i].key == kv_pair.key) {//当前槽位存在键值对 且key与需要更新的kv对的key相同
            ...
            //修改后需要立即持久化
            PersistWorker::post_persist(&(tar_bucket->slot[i].value), sizeof(tar_bucket->slot[i].value));//只需持久化value而不用持久化整个桶 保障运行性能
            return 0;
        }
    }
...
void PmEHash::splitBucket(uint64_t bucket_id) {
    ...
    for(int i=0;i<BUCKET_SLOT_NUM;i++) if (hashFunc(sp_Bucket->slot[i].key)&(1<<old_local_depth)){
        ...
        PersistWorker::post_persist(&(new_Bucket->slot[cur]),sizeof(kv));//将桶中的键值对持久化
        cur++;
    }
    ...
    for(int i=(bucket_id&((1<<old_local_depth)-1))|(1<<old_local_depth);i<metadata->catalog_size;i+=(1<<(old_local_depth+1))){//更新目录项，一半指向旧桶的目录项指向新桶
        ...
        PersistWorker::post_persist(&(catalog->buckets_pm_address[i]), sizeof(pm_address));
    }
